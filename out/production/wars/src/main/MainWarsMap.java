...
public class MainWarsMap extends JFrame implements AddLayerDialog.AddLayerInterface,
    ...
    public MainWarsMap() {
        ...
        Utils.addShapeFileToMap(mMap, "/Users/xxy/Desktop/IdeaProjects/wars/data/ShapeFile/basemap.shp");
        initToc();
    ...
    private void initMenuBar() {
        ...
        JMenu layerControlMenu = new JMenu("Layer Control");
        ...
        JMenuItem readItem = new JMenuItem("Read Me");
        ...
        helpMenu.add(readItem);
        languageMenu.add(enItem);
        languageMenu.add(cnItem);
...
//                    showHelpDialog(Constants.PRINT_HELP);
    ...
    private void translate(Locale locale) {
        ResourceBundle resourceBundle = ResourceBundle.getBundle("NamesBundle", locale);
        for (int i = 0; i < mMenuBar.getMenuCount(); i++) {
            JMenu menu = mMenuBar.getMenu(i);
            menu.setText(resourceBundle.getString(Constants.NAME_BUNDLE_KEYS_MENU[i]));

            switch (menu.getText()) {
                case "File":
                case "文件":
                    for (int j = 0; j < menu.getItemCount(); j++) {
                        menu.getItem(j).setText(resourceBundle.getString(Constants.NAME_BUNDLE_KEYS_FILE[j]));
                    }
                    break;
                case "Theme":
                case "主题":
                    for (int j = 0; j < menu.getItemCount(); j++) {
                        menu.getItem(j).setText(resourceBundle.getString(Constants.NAME_BUNDLE_KEYS_THEME[j]));
                    }
                    break;
                case "Layer Control":
                case "控制图层":
                    for (int j = 0; j < menu.getItemCount(); j++) {
                        menu.getItem(j).setText(resourceBundle.getString(Constants.NAME_BUNDLE_KEYS_LAYER[j]));
                    }
                    break;
                case "Help":
                case "帮助":
                    for (int j = 0; j < menu.getItemCount(); j++) {
                        JMenuItem helpMenuItem = menu.getItem(j);
                        helpMenuItem.setText(resourceBundle.getString(Constants.NAME_BUNDLE_KEYS_HELP[j]));
                        if (helpMenuItem instanceof JMenu && (helpMenuItem.getText().equals("Help Topics") || helpMenuItem.getText().equals("帮助释义"))) {
                            JMenu topicMenu = (JMenu) helpMenuItem;
                            for (int p = 0; p < topicMenu.getItemCount(); p++) {
                                topicMenu.getItem(p).setText(resourceBundle.getString(Constants.NAME_BUNDLE_KEYS_HELP_TOPICS[p]));
                            }
                        }
                ...
                case "语言":
                    for (int j = 0; j < menu.getItemCount(); j++) {
                        menu.getItem(j).setText(resourceBundle.getString(Constants.NAME_BUNDLE_KEYS_HELP_LANGUAGE[j]));
                    }
                    break;
                default:
                    break;
